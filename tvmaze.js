...
let response = await axios(`https://api.tvmaze.com/search/shows?q=${query}`);
...
let response = await axios(`https://api.tvmaze.com/shows/${id}/episodes`);
let episode = response.data.map(function(episode){
return {
  id: episode.id,
  name: episode.name,
